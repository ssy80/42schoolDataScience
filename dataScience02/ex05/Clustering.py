...
def plot_bubble_chart(customers_features_df):
    ...
    max_median_freq = summary_df["median_freq"].max()
    y_ticks = np.arange(0, max_median_freq + 2, 1)
    plt.ylim(0, max_median_freq + 2)
    plt.yticks(y_ticks)
...
def cluster_customers(customers_df):
    """Kmeans cluster by recency, frequency into new, regular, inactive, then
    Kmeans cluster regular segment spending into high, low spending.
    """
